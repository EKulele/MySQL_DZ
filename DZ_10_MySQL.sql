-- 1.1 Создайте таблицу logs типа Archive. Пусть при каждом создании записи в таблицах users, catalogs и products
...
delimiter //
create trigger log_after_insert_to_users after insert on users2
	for each row begin
		insert into logs (tablename, extenal_id, name)
		values ('users2', new.id, new.name);
	end //
...
insert into catalogs(name) values ('Жесткие блинчики');
